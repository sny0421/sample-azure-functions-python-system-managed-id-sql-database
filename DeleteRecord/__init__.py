...
def main(req: func.HttpRequest) -> func.HttpResponse:
    logging.info('Start function: DeleteRecord')

    try:
        db_id = req.params.get('id')
    except:
        return func.HttpResponse(
            "Please put a valid parameter in the request",
            status_code=400
        )
    ...
    try:
        ...
        cursor = conn.cursor()
        # Execute SQL query
        cursor.execute("DELETE FROM test WHERE id = ?", db_id)
        cursor.commit()
